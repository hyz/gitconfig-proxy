...
pub struct Subcommand {
    ...
    #[clap(parse(try_from_str = url_parse))]
    url: Url,
...
impl Runnable for Subcommand {
    ...
    fn run(&self) {
        ...
        futures::executor::block_on(self.clone_or_pull()).unwrap();
        //clone_or_pull().await?;
        //https2git().await
    }
}
...
impl Subcommand {
    async fn clone_or_pull(&self) -> Result<()> {
        ...
        git_clone(&["--depth", "1"], &fixed_url, self.path.as_ref())
            .await?
            .exit_ok()?;

        let lcd = self.path.as_ref().map_or_else(
